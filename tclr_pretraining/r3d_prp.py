...
class SpatioTemporalResBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # print()
        # print(f'Res shape after conv1 is {res.shape}')
        res = self.bn2(self.conv2(res))

        if self.downsample:
            x = self.downsamplebn(self.downsampleconv(x))
        # print(f'x shape is {x.shape}, res shape is {res.shape}')
...
class R3DNet(nn.Module):
    ...
    def __init__(self, layer_sizes, block_type=SpatioTemporalResBlock, with_classifier=False, return_conv=True, num_classes=101):
    ...
    def forward(self, x):
        with autocast():
            ...
            x = self.conv3(x)
            x = self.conv4(x)
            # print(f'After 4th conv {x.shape}')

            x = self.conv5(x)
            # print(f'After 5th conv {x.shape}')
